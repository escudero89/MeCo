...
function [ phi ] = placa_2d (
    ...
    # Salteo los nodos del borde interior y exterior
    pos_en_malla += 2 * cant_nodo_en_dir + 1;
    
    # Generamos un vector que nos dice si un nodo es interior (1), o de borde ext-int (0)
    es_interior = zeros(length(phi), 1);
    
    # Hasta length(phi) - cant_nodo_en_dir porque salteamos la ultima parte que son nodos de borde.
    for pos = pos_en_malla : length(phi) - 2 * cant_nodo_en_dir - 1
