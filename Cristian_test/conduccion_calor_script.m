1;
clear all;
clf;
close;

Lx = 1;
dx = 0.01;
dt = 1000;
t_f = dt * 150;
tipo_cond = [0 0];
val_cond = [1 0];

f = 5;
phi_0 = sin(f * pi .* (0:dx:Lx)');

%[PHI_1] = conduccion_calor_no_estacionario_1d( 1, phi_0, Lx, dx, t_f, dt, tipo_cond, val_cond );

[PHI_1] = conduccion_calor_no_estacionario_1d( 2, phi_0, Lx, dx, t_f, dt, tipo_cond, val_cond );

%[PHI_3] = conduccion_calor_no_estacionario_1d( 3, phi_0, Lx, dx, t_f, dt, tipo_cond, val_cond );

cant_N = 1;
N = cant_N * floor(size(PHI_1)(1) / cant_N);
N = 20;

#subplot (1, 3, 1)

for i = 1 : length(0 : dt : t_f)

plot(0:dx:Lx, PHI_1(i,:));
grid;
xlabel('x');
ylabel('phi');
title('Fordward Euler');
axis('equal');
pause
i
end

#subplot (1, 3, 2)
#plot(0:dx:Lx, PHI_2(1:cant_N:N,:));
#grid;
#xlabel('x');
#ylabel('phi');
#title('Backward Euler');
#axis('equal');

#subplot (1, 3, 3)
#plot(0:dx:Lx, PHI_3(1:cant_N:N,:));
#grid;
#xlabel('x');
#ylabel('phi');
#title('Crank-Nicholson');
#axis('equal');
