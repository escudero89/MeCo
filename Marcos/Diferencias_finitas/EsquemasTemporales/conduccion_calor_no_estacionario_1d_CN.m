...
## Created: 2013-09-13
##------------------------------------------------------------------------------
function [PHI] = conduccion_calor_no_estacionario_1d_CN(phi0, Lx, dx, dt, t_f,
    ...
    for( i = 1 : length(cant_x) )
        diagonal_central =  [diagonal_central, (-1/dt - k(cant_x(i))/dx2 + c(cant_x(i))/2)];
        diagonal_superior = [diagonal_superior, (k(cant_x(i))/(2*dx2))];
        diagonal_inferior = [diagonal_inferior, (k(cant_x(i))/(2*dx2))];
    endfor
    ...
    for n = 1 : t_iter - 1
        ...
        if( tipo_cond(1) )  # Contorno Neumann en x = 0
            ...
            ( Q(cant_t(n),cant_x(1)) + Q(cant_t(n+1),cant_x(1)) )/2 ...
            + PHI(n,1)*(-1/dt + k(cant_x(1))/dx2 - c(cant_x(1))/2) - ...
            PHI(n,2) * k(cant_x(1))/dx2 - val_cond(n,1)/dx - val_cond(n+1,1)/dx;
        ...
        if( tipo_cond(2) )  # Contorno Neumann en x = 0
            ...
            f(end) = c(cant_x(end)) * phi_amb(cant_x(end)) - ...
            ( Q(cant_t(n),cant_x(end)) + Q(cant_t(n+1),cant_x(end)) )/2 ...
            + PHI(n,end)*(-1/dt + k(cant_x(end))/dx2 - c(cant_x(end))/2) - ...
            PHI(n,end-1) * k(cant_x(end))/dx2 + val_cond(n,end)/dx + val_cond(n+1,end)/dx;    
        ...
        for(i = 2 : (length(cant_x)-1) )
            ...
            ( Q(cant_t(n),cant_x(i)) + Q(cant_t(n+1),cant_x(i)) )/2 +...
            PHI(n,i)*(-1/dt + k(cant_x(i))/dx2 - c(cant_x(i))/2) - ...
            PHI(n,i-1) * k(cant_x(i))/(2*dx2) - PHI(n,i+1) * k(cant_x(i))/(2*dx2);
